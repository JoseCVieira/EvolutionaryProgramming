...
public class Simulation {
	...
	void simulateEvent(Event current_event){
		...
		if(current_event.action() == 'M'){
			...
			pec.addEvent(new EvMove(current_time + expRandom(move_param), current_event.individual));
	...
	void createNewBornEvents(Individual i){
		pec.addEvent(new EvDeath(current_time + expRandom(death_param), i));
		pec.addEvent(new EvReproduction(current_time + expRandom(reprod_param),i));
		pec.addEvent(new EvMove(current_time + expRandom(move_param), i));
	}
